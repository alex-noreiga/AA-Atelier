...
//
// Two features still hinge on a hardcoded option *value* — the sellable status
// (STATUS_IN_STOCK) and the measurement-lock stage (MEASUREMENT_LOCK_FROM_STAGE).
// These are deliberate exceptions to the "never hardcode a Notion option list"
// rule (see CLAUDE.md), but they share a failure mode: when someone renames or
// removes that option in Notion, the frozen name stops matching and the feature
// quietly breaks — with no error, no test failure, nothing. This module makes
// that loud.
//
// (The shop's size chart used to be a third such name — see the removed
// SIZED_CATEGORY_NAMES — but it is now Notion-driven via the Product Categories
// relation, so there is no name left to drift.)
//
// The audit is ADVISORY: it computes what's missing and callers log it. It never
// throws and never changes behaviour.
