...
1. **Only non-secret tunables live here.** Secrets (`NOTION_API_KEY`,
   ...
   wiring (every `NOTION_*_DATABASE_ID`, `SUPABASE_URL`, `PUBLIC_BASE_URL`, …)
   stay in Vercel — a Notion DB is
...
2. **Rush is derived from the date + an explicit acknowledgement.** `isRushNeededBy`
   ...
   ticked), and sends `rush: true` on the order. Both the date and the disclosure
   live on the intake's **last** step, so the surcharge is the last thing the
   customer reads before placing the order rather than a mid-form interruption. A
   standard-timeline date sends no `rush` field. The `rush` boolean is part of the
   OpenAPI contract (`NewOrderRequest.rush`, contract-first + generated client).
...
3. **Flat multi-select, controlled.** `ColorPicker` (`components/color-picker.tsx`)
   ...
   existing **Reference Images** upload, alongside it on the design step (no
   separate uploader).
