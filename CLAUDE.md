...
Express app (artifacts/api-server)  ──►  Notion REST API (orders database)
  ...
  │                                  bespoke commissions, the atelier's own
  │                                  closed-books wording, and the dated
  │                                  competitions a waitlist entry can be pinned
  │                                  to. The same decision gates POST /orders.
  │                                  Fails OPEN — see "Seasonal capacity"
  ├─ POST /api/waitlist            → joins the commission waitlist (name + email,
  │                                  optional phone, event/needed-by and a note),
  │                                  filed in the contact database tagged
  │                                  Request type = "Waitlist". Accepted whether
  │                                  or not the books are closed; never creates
  │                                  an order and never holds a slot
...
## Seasonal capacity & the commission waitlist
...
`lib/notion/competitions.{schema,repository}.ts`, `listOpenOrderServices` in
...
8. **Competitions is read for what it can answer, and not for what it can't.** The
   "🏆 Competitions" database dates the events customers skate, so the waitlist
   offers them as a picker and the entry records the date the piece is genuinely
   needed. Its `Push starts` formula answers "when should we start advertising",
   which is a different question, and the app never reads it. A row with no name
   or no date is **dropped** rather than rendered as a blank option — which is the
   state the database is in today, so the form currently asks for a plain date
   instead. That fallback is the normal path, not an error case. Read-only,
   60s-cached, and degrading to `[]` on any failure (an unset id, an unshared
   integration) so a waitlist capture can never be lost to it.

9. **The waitlist needs no new Notion property, and never trusts a client label.**
   It is the seventh writer to the contact database
   (`Request type = "Waitlist"`), and the event it is for reuses the shared
   **`Item`** rich_text — so the atelier can group the waitlist by competition in a
   Notion view without adding a column. A picked `eventId` is resolved server-side
   against the live competition list and the event's **own** name and date are
   stored; the browser's `eventName` is used only when the id resolves to nothing.
   Best-effort customer acknowledgement **and** an atelier notification (unlike a
   newsletter opt-in — a waitlist entry is somebody asking for work, and is the
   signal the books reopen on) plus the usual best-effort CRM `Lead`.
...
**Atelier setup: none required.** The cap defaults to no cap, so the feature is
inert until a number is entered under `/studio` → **Studio settings** (three
tunables: **Commission intake**, **Commissions in production at once**,
**Closed-books message**). Optional, for the seasonal half: set
**`NOTION_COMPETITIONS_DATABASE_ID`**, share the Notion integration with **🏆
Competitions**, and fill in `Date` (and ideally `Season`) on its rows — until
then the waitlist asks for a date. The `Waitlist` `Request type` option
auto-creates on first write. Full detail in
`.agents/memory/seasonal-capacity-waitlist.md`.
...
- **Optional competitions database:** `NOTION_COMPETITIONS_DATABASE_ID` (the "🏆
  Competitions" database). When set (and the integration is shared with it), the
  commission waitlist offers the studio's dated upcoming competitions as a
  picker, so an entry records the event the piece is actually for. Read-only and
  degrade-safe: unset, unshared, or holding no dated future rows ⇒ the waitlist
  asks for a plain date instead, which is the state it is in today. Read at first
  use in `getCompetitionsNotionClient`. See "Seasonal capacity & the commission
  waitlist" above.
- **Optional commission-capacity env vars:** `COMMISSION_CAPACITY` (default `0` =
  ...
  missing or unusable value can never shut the order form.
...
| `NOTION_COMPETITIONS_DATABASE_ID`                                                                                 | The waitlist asks for a date instead of offering an event picker    |
...
| Change seasonal capacity / the commission waitlist       | `api-server/src/services/capacity.ts` (the pure `resolveIntake` + the three setting getters) + `services/capacity.service.ts` (the cached count + `GET /capacity`) + `listOpenOrderServices` in `lib/notion/orders.repository.ts` + `enforceCapacityGate` in `services/orders.service.ts`; the waitlist is `services/waitlist.service.ts` + `routes/waitlist.ts` + `lib/notion/waitlist.{blocks,repository}.ts` (writes to the **contact** database) + the two `waitlist*Email` builders in `lib/resend/emails.ts`; the event picker is `lib/notion/competitions.{schema,repository}.ts`; frontend `web-app/src/components/waitlist-form.tsx` + the closed-books branch of `pages/order-form.tsx` + `capacityGated` in `lib/order-services.ts`; the studio panel is `CapacityPanel` in `pages/studio.tsx`, fed by `buildCapacity` in `services/studio-analytics.service.ts` |
